	...
	public function sendWebPushNotification(Sending $event)
	{
		...
		switch ($event->blueprint->getSubjectModel()) {
			...
			case 'Flarum\Post\Post':
				...
				$link = $url->route('discussion', ['id' => $subject->discussion_id], ['id' => $subject->post_id]);
				break;
			default:
				return;
				break;
		...
		switch ($event->blueprint->getType()){
			...
			case 'newPost':
				...
				$link = $url->route('discussion', ['id' => $subject->id], ['id' => $subject->post_id]);
		...
		try {
            ...
            $oneSignalAPI = new OneSignal($config, $client);

            $oneSignalAPI->notifications->add($notification);
